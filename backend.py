...
class Cus_manager:
    ...
    def update(self, name, phone, cid):
        query = "UPDATE Customers SET (cus_name, cus_phone) = (?, ?) where cus_id = ?"
        return exe_query(query, (name, phone, cid), commit=True)
...
class Emp_manager:
    ...
    def update(self,name, eid):
        query = "UPDATE Employees SET emp_name = ? WHERE emp_id = ?"
        return exe_query(query, (name, eid), commit=True)

class Dish_manager:
    def add(self, name, cooking_time, price):
        query = "INSERT INTO Dishes (dish_name, cooking_time, dish_price) VALUES (?, ?, ?)"
        return exe_query(query, (name, cooking_time, price), commit=True)
    ...
    def list_dishes(self):
        query = "SELECT dish_id, dish_name, cooking_time, dish_price FROM Dishes"
        return exe_query(query)

    def update_dish(self, name, time, price, dish_id):
        query = "UPDATE Dishes SET (dish_name, cooking_time, dish_price) = (?, ?, ?) WHERE dish_id = ?"
        return exe_query(query, (name, time, price, dish_id), commit=True)

class Order_manager:
    def create_orders(self, dish_req_in, total_price, cus_id):
        dish_req_json = json.dumps(dish_req_in)
        ...
        try:
            cur.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (dish_req_json, total_price, order_time, status, cus_id))
...
class Ingredient_manager:
    ...
    def find_ingr(self, keyword):
        query = f"""
            SELECT ingre_id, ingre_name, stock, unit, expiry, suppliers
            FROM Ingredients
            WHERE ingre_name LIKE "{keyword}"
        """
        params = ()
        return exe_query(query, params, fetch_one=True)
